...
export function InsightCards({ analytics, questions }: {
  ...
  if (analytics.completionRate >= 0.75) {
    insights.push({
      icon: TrendingUp, color: "text-emerald-700 dark:text-emerald-400", bg: "bg-emerald-500/10 border-emerald-500/20",
    ...
    insights.push({
      icon: AlertTriangle, color: "text-amber-700 dark:text-amber-400", bg: "bg-amber-500/10 border-amber-500/20",
  ...
  if (worstDropoff) {
    ...
    if (q) {
      insights.push({
        icon: AlertTriangle, color: "text-red-700 dark:text-red-400", bg: "bg-red-500/10 border-red-500/20",
  ...
  if (analytics.mobilePercentage > 0.55) {
    insights.push({
      icon: Smartphone, color: "text-blue-700 dark:text-blue-400", bg: "bg-blue-500/10 border-blue-500/20",
  ...
  if (npsQuestion && npsQuestion.npsScore !== undefined) {
    ...
    const bg = score >= 50 ? "bg-emerald-500/10 border-emerald-500/20" : score >= 0 ? "bg-amber-500/10 border-amber-500/20" : "bg-red-500/10 border-red-500/20"
    insights.push({
  ...
  if (analytics.totalResponses < 3) {
    insights.push({
      icon: Users, color: "text-muted-foreground", bg: "bg-muted/40 border-muted/50",
  ...
  return (
    <div className="space-y-3 mb-6">
      {insights.map((ins, i) => (
        <div key={i} className={`flex items-start gap-3.5 rounded-xl border p-4 shadow-sm ${ins.bg}`}>
