...
export const FORM_TEMPLATES: FormTemplate[] = [
  ...
  // ─── Formulário de Contato ────────────────────────────────────────────────
  {
    ...
    questions: [
      ...
      { type: "whatsapp", order: 3, required: false, title: "Qual é o seu WhatsApp? (opcional)" },
  ...
  // ─── Candidatura de Emprego ───────────────────────────────────────────────
  {
    ...
    questions: [
      ...
      { type: "email", order: 2, required: true, title: "Qual é o seu e-mail?" },
      { type: "cpf", order: 3, required: true, title: "Qual é o seu CPF (para fins de ficha cadastral)?" },
      { type: "whatsapp", order: 4, required: true, title: "Qual é o seu WhatsApp?" },
      { type: "short_text", order: 5, required: true, title: "Para qual vaga você está se candidatando?" },
      { type: "multiple_choice", order: 6, required: true, title: "Qual é o seu nível de experiência?", properties: opt(["Sem experiência (Trainee/Estágio)", "Júnior (0–2 anos)", "Pleno (2–5 anos)", "Sênior (5+ anos)", "Especialista / Lead"]) },
      { type: "scale", order: 7, required: true, title: "Como você avalia seu inglês?", properties: { scaleMin: 1, scaleMax: 5, scaleMinLabel: "Básico", scaleMaxLabel: "Fluente" } },
      { type: "url", order: 8, required: false, title: "Cole o link do seu LinkedIn ou portfólio" },
      { type: "long_text", order: 9, required: true, title: "Por que você quer trabalhar conosco?" },
      { type: "thank_you", order: 10, required: false, title: "Candidatura recebida!", description: "Analisaremos seu perfil e entraremos em contato em até 5 dias úteis." },
    ],
  },
  ...
  // ─── Inscrição em Evento ──────────────────────────────────────────────────
  {
    ...
    questions: [
      ...
      { type: "cpf", order: 3, required: true, title: "Qual é o seu CPF? (necessário para emissão do certificado)" },
      { type: "whatsapp", order: 4, required: false, title: "WhatsApp para atualizações (opcional)" },
      { type: "short_text", order: 5, required: false, title: "Empresa / Organização" },
      { type: "multiple_choice", order: 6, required: true, title: "Como você ficou sabendo do evento?", properties: opt(["E-mail marketing", "Redes sociais", "Indicação", "LinkedIn", "Site do evento"]) },
      { type: "yes_no", order: 7, required: true, title: "Você precisa de nota fiscal para participar?" },
      { type: "thank_you", order: 8, required: false, title: "Inscrição confirmada!", description: "Você receberá um e-mail de confirmação em breve. Nos vemos lá!" },
    ],
  },
  ...
  // ─── Solicitação de Orçamento ─────────────────────────────────────────────
  {
    ...
    questions: [
      ...
      { type: "whatsapp", order: 3, required: true, title: "WhatsApp para contato rápido" },
      { type: "cnpj", order: 4, required: true, title: "CNPJ da sua empresa" },
  ...
  // ─── Triagem Odontológica ─────────────────────────────────────────────────
  {
    ...
    questions: [
      ...
      { type: "cpf", order: 2, required: true, title: "Qual é o seu CPF? (Para ficha de saúde)" },
      { type: "date", order: 3, required: true, title: "Data de nascimento" },
      { type: "whatsapp", order: 4, required: true, title: "WhatsApp para contato da clínica" },
      { type: "multiple_choice", order: 5, required: true, title: "Qual é o principal motivo da sua consulta?", properties: opt(["Dor de dente", "Sangramento na gengiva", "Dente quebrado ou lascado", "Revisão / Limpeza", "Aparelho ortodôntico", "Clareamento", "Outro"]) },
      { type: "scale", order: 6, required: false, title: "Se sentir dor, qual a intensidade?", properties: { scaleMin: 0, scaleMax: 10, scaleMinLabel: "Sem dor", scaleMaxLabel: "Dor insuportável" } },
      { type: "multiple_choice", order: 7, required: true, title: "Há quanto tempo apresenta esse problema?", properties: opt(["Menos de 24 horas", "1 a 3 dias", "1 semana", "Mais de 1 semana", "Não tenho dor"]) },
      { type: "checkbox", order: 8, required: false, title: "Possui alguma dessas condições de saúde?", properties: opt(["Diabetes", "Hipertensão", "Problemas cardíacos", "Alergia a anestesia ou medicamentos", "Gestante", "Usa anticoagulantes", "Nenhuma das anteriores"]) },
      { type: "yes_no", order: 9, required: true, title: "Já realizou algum tratamento odontológico anteriormente?" },
      { type: "long_text", order: 10, required: false, title: "Alguma informação adicional que o dentista deva saber?" },
      { type: "thank_you", order: 11, required: false, title: "Triagem concluída!", description: "Suas informações foram recebidas. Por favor, aguarde ser chamado." },
    ],
  },
  ...
  // ─── Triagem Médica ───────────────────────────────────────────────────────
  {
    ...
    questions: [
      ...
      { type: "short_text", order: 1, required: true, title: "Nome completo do paciente" },
      { type: "cpf", order: 2, required: true, title: "Qual é o seu CPF? (Para o prontuário da clínica)" },
      { type: "date", order: 3, required: true, title: "Data de nascimento" },
      { type: "whatsapp", order: 4, required: true, title: "WhatsApp" },
      { type: "multiple_choice", order: 5, required: true, title: "Qual é o motivo da consulta?", properties: opt(["Consulta de rotina / Check-up", "Sintomas agudos (febre, dor, etc.)", "Acompanhamento de doença crônica", "Renovação de receita", "Resultado de exame", "Outro"]) },
      { type: "long_text", order: 6, required: true, title: "Descreva seus sintomas principais", description: "Onde dói, quando começou, como é a dor." },
      { type: "scale", order: 7, required: false, title: "Qual a intensidade dos sintomas?", properties: { scaleMin: 1, scaleMax: 10, scaleMinLabel: "Leve", scaleMaxLabel: "Muito intenso" } },
      { type: "multiple_choice", order: 8, required: true, title: "Há quanto tempo apresenta esses sintomas?", properties: opt(["Menos de 24 horas", "1 a 3 dias", "4 a 7 dias", "Mais de 1 semana", "Mais de 1 mês"]) },
      { type: "checkbox", order: 9, required: false, title: "Possui alguma doença crônica diagnosticada?", properties: opt(["Diabetes", "Hipertensão", "Doença cardíaca", "Asma / DPOC", "Tireoide", "Depressão / Ansiedade", "Nenhuma"]) },
      { type: "yes_no", order: 10, required: true, title: "Está tomando algum medicamento atualmente?" },
      { type: "long_text", order: 11, required: false, title: "Se sim, quais medicamentos e doses?" },
      { type: "yes_no", order: 12, required: true, title: "Possui alergia a algum medicamento?" },
      { type: "thank_you", order: 13, required: false, title: "Triagem concluída!", description: "Em breve você será chamado para o atendimento." },
    ],
  },
  ...
  // ─── Triagem Oficina ──────────────────────────────────────────────────────
  {
    ...
    questions: [
      ...
      { type: "cpf", order: 2, required: true, title: "Qual é o seu CPF (para fins de Nota Fiscal)?" },
      { type: "whatsapp", order: 3, required: true, title: "WhatsApp" },
      { type: "email", order: 4, required: false, title: "E-mail (para envio do orçamento)" },
      { type: "short_text", order: 5, required: true, title: "Marca e modelo do veículo", description: "Ex: Toyota Corolla, Volkswagen Gol" },
      { type: "short_text", order: 6, required: true, title: "Ano de fabricação" },
      { type: "short_text", order: 7, required: true, title: "Placa do veículo" },
      { type: "number", order: 8, required: false, title: "Quilometragem atual (km)" },
      { type: "multiple_choice", order: 9, required: true, title: "Qual é o tipo de serviço solicitado?", properties: opt(["Revisão preventiva", "Troca de óleo e filtros", "Freios", "Suspensão e direção", "Motor / Câmbio", "Elétrica / Eletrônica", "Funilaria e pintura", "Diagnóstico (não sei o problema)"]) },
      { type: "long_text", order: 10, required: true, title: "Descreva o problema ou o que está acontecendo com o veículo", description: "Quanto mais detalhes, melhor podemos ajudar." },
      { type: "multiple_choice", order: 11, required: true, title: "Quando o problema começou?", properties: opt(["Hoje", "Esta semana", "Este mês", "Faz mais de 1 mês", "Nunca reparei antes"]) },
      { type: "yes_no", order: 12, required: true, title: "Autoriza orçamento antes de executar o serviço?" },
      { type: "thank_you", order: 13, required: false, title: "Triagem recebida!", description: "Nossa equipe entrará em contato em breve com o diagnóstico e orçamento." },
    ],
  },
  ...
  // ─── Voluntários de Campanha ──────────────────────────────────────────────
  {
    ...
    questions: [
      ...
      { type: "short_text", order: 1, required: true, title: "Qual é o seu nome completo?" },
      { type: "cpf", order: 2, required: true, title: "Qual é o seu CPF (exigência do TSE)?" },
      { type: "email", order: 3, required: true, title: "Qual é o seu e-mail?" },
      { type: "whatsapp", order: 4, required: true, title: "WhatsApp para contato" },
      { type: "short_text", order: 5, required: true, title: "Em qual cidade e estado você mora?" },
      { type: "multiple_choice", order: 6, required: true, title: "Qual é a sua disponibilidade para o voluntariado?", properties: opt(["Fins de semana", "Noites de semana", "Horário comercial", "Tempo integral", "Somente online"]) },
      { type: "checkbox", order: 7, required: true, title: "Como você quer ajudar? (pode marcar mais de um)", properties: opt(["Panfletagem e abordagem", "Redes sociais e conteúdo digital", "Organização de eventos", "Transporte de materiais", "Apoio administrativo", "Captação de recursos", "Boca a boca na comunidade"]) },
      { type: "checkbox", order: 8, required: false, title: "Quais habilidades você tem?", properties: opt(["Design gráfico", "Fotografia / Vídeo", "Comunicação e oratória", "Gestão de projetos", "Programação / Tecnologia", "Jornalismo / Redação", "Nenhuma específica"]) },
      { type: "short_text", order: 9, required: false, title: "Qual bairro ou região você pode cobrir?" },
      { type: "long_text", order: 10, required: false, title: "Por que você quer apoiar essa campanha?", description: "Conta um pouco sobre você e suas motivações." },
      { type: "thank_you", order: 11, required: false, title: "Bem-vindo ao time!", description: "Seu cadastro foi recebido. Em breve entraremos em contato com as próximas etapas." },
    ],
  },
  ...
  // ─── Qualificação de Lead B2B ─────────────────────────────────────────────
  {
    id: "b2b-lead-qualification",
    title: "Qualificação de Lead B2B",
    description: "Qualifique empresas automaticamente via WhatsApp antes de passá-las para o seu time de vendas.",
    category: "vendas",
    color: "from-blue-700 to-indigo-900",
    accent: "#1d4ed8",
    estimatedTime: "~3 min",
    questions: [
      { type: "welcome", order: 0, required: false, title: "Análise de Especialista", description: "Preencha os dados abaixo para que nosso time avalie como podemos escalar sua operação." },
      { type: "short_text", order: 1, required: true, title: "Qual é o seu nome?" },
      { type: "whatsapp", order: 2, required: true, title: "Qual é o seu WhatsApp profissional?" },
      { type: "cnpj", order: 3, required: true, title: "Qual o CNPJ da sua empresa?" },
      { type: "multiple_choice", order: 4, required: true, title: "Qual o seu cargo atual?", properties: opt(["Sócio / Fundador (C-Level)", "Diretor / Gerente", "Coordenador / Analista", "Outro"]) },
      { type: "multiple_choice", order: 5, required: true, title: "Qual o faturamento médio mensal?", properties: opt(["Ainda não faturo", "Até R$ 50 mil", "R$ 50k a R$ 200k", "R$ 200k a R$ 1 Milhão", "Mais de R$ 1 Milhão"]) },
      { type: "long_text", order: 6, required: true, title: "Qual é o principal desafio (gargalo) da sua empresa hoje?" },
      { type: "thank_you", order: 7, required: false, title: "Tudo pronto!", description: "Um de nossos especialistas entrará em contato com você pelo WhatsApp em breve." },
    ],
  },

  // ─── Interesse de Imóvel (Corretor) ───────────────────────────────────────
  {
    id: "real-estate-interest",
    title: "Ficha de Interesse de Imóvel",
    description: "Capture os dados de potenciais compradores/locatários diretamente para o seu WhatsApp.",
    category: "vendas",
    color: "from-emerald-600 to-teal-800",
    accent: "#059669",
    estimatedTime: "~2 min",
    questions: [
      { type: "welcome", order: 0, required: false, title: "Encontre o imóvel ideal!", description: "Conte-nos o que você procura e enviaremos as melhores opções disponíveis." },
      { type: "short_text", order: 1, required: true, title: "Qual é o seu nome?" },
      { type: "whatsapp", order: 2, required: true, title: "Seu melhor WhatsApp para receber as opções" },
      { type: "multiple_choice", order: 3, required: true, title: "Você busca imóvel para:", properties: opt(["Comprar", "Alugar", "Investir"]) },
      { type: "multiple_choice", order: 4, required: true, title: "Qual tipo de imóvel?", properties: opt(["Apartamento", "Casa / Sobrado", "Cobertura", "Terreno / Lote", "Comercial / Sala"]) },
      { type: "multiple_choice", order: 5, required: true, title: "Quantos quartos?", properties: opt(["1 Quarto", "2 Quartos", "3 Quartos", "4+ Quartos", "Não se aplica"]) },
      { type: "multiple_choice", order: 6, required: false, title: "Qual sua previsão máxima de investimento?", properties: opt(["Até R$ 300 mil", "Até R$ 500 mil", "Até R$ 1 Milhão", "Acima de R$ 1 Milhão", "Aluguel (Até R$ 3 mil)", "Aluguel (Mais de R$ 3 mil)"]) },
      { type: "long_text", order: 7, required: false, title: "Tem algum bairro de preferência ou exigência específica?", description: "Ex: Precisa aceitar pet, 2 vagas de garagem, sol da manhã..." },
      { type: "thank_you", order: 8, required: false, title: "Ficha recebida!", description: "Um de nossos corretores especialistas vai te chamar no WhatsApp com a seleção de imóveis!" },
    ],
  },
]
